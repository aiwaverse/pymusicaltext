...
def text(label: str, font_weight="bold", key='', size=(None, None)) -> Text:
    return sg.Text(label, size=size, text_color="#fff", font=("Roboto", 11, font_weight), key=key)
...
class Layout:
    ...
    def make_gui(self) -> sg.Window:
        ...
        input_text_col = sg.pin(sg.Column([
            [text("Type a Text")],
            [
                sg.Multiline(
                    font=("Ubuntu", 11),
            ...
            [sg.Input(size=(25, 20), font=("Ubuntu", 12), key=gui_constants.IN_FILE_INPUT),
             sg.FileBrowse(font=("Ubuntu", 12))],
        ], size=(370, 300), element_justification="left", vertical_alignment="top"))

        file_name_col = sg.pin(sg.Column([
        ...
        file_info_col = sg.pin(sg.Column([
            [text("Dados da arquivo:")],
            [text("Nome:", "normal"), text("Not load", "normal", gui_constants.TEXT_FILE_NAME, size=(30, 1))],
            [text("Duração:", "normal"), text("", "normal", gui_constants.TEXT_DURATION, size=(30, 1))],
            [text("Data de Criação:", "normal"), text("", "normal", gui_constants.TEXT_CREATED_AT, size=(30, 1))],
        ], size=(370, 120), element_justification="left", vertical_alignment="top", visible=False,
            key=gui_constants.FILE_INFO_SECTION))

        player_col_bg = '#335267'

        player_col = sg.pin(sg.Column([
            ...
            visible=False, key=gui_constants.PLAYER_SECTION))
